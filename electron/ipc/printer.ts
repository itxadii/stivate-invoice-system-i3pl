...
const buildChallanPage = (pdfDoc: PDFDocument, dispatch: any, font: any, fontBold: any, logo: any) => {
  ...
  page.drawText('I3PL INDIA PVT LTD', { x: textX, y: yCursor - 20, size: 11, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
  page.drawText('Gat No. 1462/63, Dhoksangavi,', { x: textX, y: yCursor - 32, size: 7.5, font, color: rgb(0.3, 0.3, 0.3) });
  page.drawText('Tal-Shirur, Dist-Pune, Maharashtra-412209', { x: textX, y: yCursor - 42, size: 7.5, font, color: rgb(0.3, 0.3, 0.3) });
  page.drawText('Contact Number : +918625866581', { x: textX, y: yCursor - 52, size: 7.5, font, color: rgb(0.3, 0.3, 0.3) });
  page.drawText('E-mail : kitpulling.b-warehouse@i3plindia.com', { x: textX, y: yCursor - 62, size: 7.5, font, color: rgb(0.3, 0.3, 0.3) });
  ...
  page.drawText('GSTIN: 27AACCP7114K1ZB', { x: rightX, y: yCursor - 20, size: 9, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
  page.drawText(`Challan No: EL/${dispatch.dc_no || 'DRAFT'}/2026-27`, { x: rightX, y: yCursor - 38, size: 10, font: fontBold, color: rgb(0.1, 0.1, 0.1) });

  let formattedDate = dispatch.date || new Date().toISOString().split('T')[0];
  try {
    const dObj = new Date(dispatch.date || new Date().toISOString());
    if (!isNaN(dObj.getTime())) {
      const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
      formattedDate = `${dObj.getDate()}-${months[dObj.getMonth()]}-${dObj.getFullYear().toString().slice(-2)}`;
    }
  } catch (e) {
    console.error('Date format issue while building challan page:', e);
  }
  page.drawText(`Date: ${formattedDate}`, { x: rightX, y: yCursor - 56, size: 9, font, color: rgb(0.3, 0.3, 0.3) });
  ...
  page.drawText('Consignee/ Transfer To,', { x: margin + 10, y: yCursor - 15, size: 9, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
  const addressLines = ((dispatch.address || 'AS PER LIST') as string).split('\n');
  let addrY = yCursor - 27;
  for (const line of addressLines) {
    page.drawText(line, { x: margin + 10, y: addrY, size: 8, font, color: rgb(0.2, 0.2, 0.2) });
    addrY -= 11;
  }
  ...
  page.drawText(`F W H TO ${plantName}`, { x: rightX, y: yCursor - 25, size: 10, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
  page.drawText(`VEHICLE NO: ${dispatch.vehicle_no || ''}`, { x: rightX, y: yCursor - 55, size: 9.5, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
  ...
  page.drawText('SR NO', { x: margin + 10, y: yCursor - 14, size: 9, font: fontBold });
  page.drawText('PARTICULAR', { x: margin + 80, y: yCursor - 14, size: 9, font: fontBold });
  page.drawText('No of Pallets', { x: margin + 420, y: yCursor - 14, size: 9, font: fontBold });
  yCursor -= 20;
  ...
  page.drawLine({ start: { x: margin + 370, y: yCursor }, end: { x: margin + 370, y: yCursor - 120 }, thickness: 1, color: rgb(0.8, 0.8, 0.8) });

  page.drawText('1', { x: margin + 20, y: yCursor - 60, size: 9, font });
  page.drawText(dispatch.particular || 'AS PER LIST', { x: margin + 80, y: yCursor - 60, size: 10, font: fontBold });
  page.drawText(String(dispatch.total_pallets || 1), { x: margin + 420, y: yCursor - 60, size: 10, font: fontBold });
  yCursor -= 120;
  ...
  page.drawLine({ start: { x: margin + 370, y: yCursor }, end: { x: margin + 370, y: yCursor - 20 }, thickness: 1, color: rgb(0.8, 0.8, 0.8) });

  page.drawText('Total', { x: margin + 80, y: yCursor - 14, size: 9, font: fontBold });
  page.drawText(String(dispatch.total_pallets || 1), { x: margin + 420, y: yCursor - 14, size: 9, font: fontBold });
  yCursor -= 20;
  ...
  page.drawText(`TRANSACTION NO : - ${dispatch.transaction_type || '0'}`, { x: margin + 15, y: yCursor - 40, size: 9.5, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
  const supName = (dispatch.supplier_name || 'MAHADEV').toUpperCase();
  page.drawText(supName, { x: margin + 220, y: yCursor - 40, size: 10, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
  page.drawText('Authorised Signatory', { x: margin + 380, y: yCursor - 100, size: 9, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
...
const buildBarcodePages = (pdfDoc: PDFDocument, items: any[], font: any, fontBold: any, logo: any, dispatch?: any) => {
  ...
  const drawHeaderAndTableHeaders = (p: any, pageIndex: number, totalPages: number) => {
    ...
    p.drawText('PULL LIST BARCODE SHEET', { x: titleX, y: yCursor - 25, size: 12, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
    ...
    const colWidths = [25, 55, 110, 60, 85, 40, 160];
    let xCursor = margin;
    ...
    p.drawText(`Page ${pageIndex} of ${totalPages}`, { x: 595.276 - margin - 60, y: 15, size: 8, font, color: rgb(0.6, 0.6, 0.6) });
  };
  ...
  for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
    ...
    for (let idx = currentItemIdx; idx < pageLimit; idx++) {
      ...
      try {
        drawBarcode(page, item.pull_list_no, xCursor + 10, yCursor - 26, 18, 0.42, 1.05);
      } catch (err) {
        console.error(`Failed to draw barcode for ${item.pull_list_no}:`, err);
      }
